...
class LiveLinkImporter():
    ...
    def set_Asset_Data(self, json_data):
        ...
        self.isFruit = bool("fruits" in self.json_data["tags"] or "fruit" in self.json_data["tags"])
        ...
        self.isSurfaceSSS = bool(("moss" or "skin" in [item.lower() for item in self.json_data["categories"]]) and self.Type == "surface")
    ...
    def initAssetImport(self):
        ...
        assetData = RendererData(self.TexturesList, self.textureTypes, self.Type, self.materialName, self.useDisplacement, self.isMetal, self.isBareMetal, self.isFruit, self.toSelRequest, self.isSpecularWorkflow, self.scanWidth, self.scanHeight, self.meta)
        render_setup = ("""""")
...
class RendererData():
    def __init__(self, textureList, textureTypes, assetType, materialName, useDisplacement, isMetal, isBareMetal, isFruit, applyToSel, isSpecular, width, height, meta):
