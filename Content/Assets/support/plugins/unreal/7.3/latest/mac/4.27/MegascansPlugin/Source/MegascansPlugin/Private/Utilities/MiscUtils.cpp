...
void ManageMaterialVersion(const FString& SelectedMaterial)
{
	...
	if (ExistingMaterialVersion == 0.0f) {
		FString MaterialPath = FPaths::Combine(TEXT("/Game"), GetMSPresetsName(), SelectedMaterial, SelectedMaterial);
		if (UEditorAssetLibrary::DoesAssetExist(MaterialPath)){
			FAssetsDatabase::Get()->AddMaterialRecord(SelectedMaterial, 1.0f);
			return;
		}
	}
